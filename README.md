...
Cutting a new release is one command — `./scripts/publish-release.sh X.Y.Z` —
but it has strict preconditions because publishing is **irreversible** (each
release is pushed to existing users' Macs over the air). There is no CI
release workflow on purpose; releases are cut by a person.

#### One-time setup (per build machine)

1. Developer ID Application certificate **for Vitgranen AB (team
   `6V9RN6W28J`)** in the login keychain. `security find-identity -v -p
   codesigning` should list it. `build-app.sh` auto-picks it first.
2. `gh` CLI installed and authenticated with write access to the repo:
   `gh auth login` → `gh auth status` clean.
3. A `ghostie-notary` keychain profile for `xcrun notarytool`, **for an
   account on the same Vitgranen AB team** (so the notary ticket matches the
   signing certificate). Recommended: an App Store Connect API key.
   ```bash
   xcrun notarytool store-credentials ghostie-notary \
     --key /path/to/AuthKey_XXXX.p8 \
     --key-id <KEY_ID> --issuer <ISSUER_UUID>
   xcrun notarytool history --keychain-profile ghostie-notary   # verify
   ```
   Override the profile name with `$NOTARY_PROFILE` if you want a different
   one.

#### Cutting a release

```bash
# 1. Land all changes for this release on main and PUSH it.
#    publish-release.sh only pushes the tag — your branch commits need to be
#    on origin already, or the GitHub Release will reference a commit that
#    isn't on the main branch.
git push origin main

# 2. Working tree must be clean (publish-release.sh aborts otherwise).
git status

# 3. Publish. Bumps the version (1.0.0 → 1.0.1 here), builds, notarizes,
#    tags, pushes the tag, creates the GitHub Release with all three assets.
./scripts/publish-release.sh 1.0.1
```

The script asks you to type the version back to confirm before doing
anything destructive (tag/push/release). On success it has produced and
uploaded:

- `Ghostie-1.0.1.zip` — the OTA payload (a stapled, notarized `Ghostie.app`,
  zipped). The in-app updater downloads exactly this.
- `Ghostie-1.0.1.sha256` — the same SHA-256 the release notes embed in
  `<!--sha256:…-->`. The updater reads the embedded one; the sidecar is for
  humans running `shasum -c`.
- `Ghostie.dmg` — for first-time installs from the GitHub Releases page.

Installed copies of Ghostie pick it up on their next daily check (or
immediately via **Settings ▸ General ▸ Updates ▸ Check Now…** or the menu's
**Check for Updates…**), verify against Apple notarization + the SHA-256 +
Team ID `6V9RN6W28J`, and swap-and-relaunch on user confirmation.

#### Common precondition failures

| Error | Fix |
|---|---|
| `git working tree is not clean` | `git status`, commit or stash the rest first. |
| `tag vX.Y.Z already exists` | You're republishing — bump the version. Tags are immutable on purpose. |
| `notary profile 'ghostie-notary' missing` | Run the `notarytool store-credentials` step above. |
| `gh CLI not found` / `run: gh auth login` | Install `gh` and authenticate. |
| Notarization rejected | Run `xcrun notarytool log <submission-id> --keychain-profile ghostie-notary` to see why. |

#### Version resolution in `build-app.sh`

For a standalone build (not via the release script), the version comes from,
in order: `--version X.Y.Z` arg → `$GHOSTIE_VERSION` env → newest git tag
(`git describe --tags --abbrev=0`) → `1.0.0`. `publish-release.sh` always
sets `$GHOSTIE_VERSION` so the build matches the tag exactly.
